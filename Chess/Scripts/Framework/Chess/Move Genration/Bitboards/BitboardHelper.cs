...
class BitboardHelper {
    ...
    public static Bitboard StraightMoves(Board board, int index) {
        ...
        Bitboard left = ((occupied & Masks.Row[row]) - (GetBitAt(index) << 1)) & Masks.Row[row];
        Bitboard right = (((occupied & Masks.Row[row]).Reverse() - (GetBitAt(index).Reverse() << 1)).Reverse()) & Masks.Row[row];

        Bitboard up = ((occupied & Masks.Column[col]) - (GetBitAt(index) << 1)) & Masks.Column[col];
        Bitboard down = (((occupied & Masks.Column[col]).Reverse() - (GetBitAt(index).Reverse() << 1)).Reverse()) & Masks.Column[col];
    ...
    public static Bitboard DiagonalMoves(Board board, int index) {
        ...
        Bitboard topRight = ((occupied & Masks.Diagonal[diagonalIndex]) - (GetBitAt(index) << 1)) & Masks.Diagonal[diagonalIndex];
        Bitboard bottomLeft = (((occupied & Masks.Diagonal[diagonalIndex]).Reverse() - (GetBitAt(index).Reverse() << 1)).Reverse()) & Masks.Diagonal[diagonalIndex];

        Bitboard topLeft = ((occupied & Masks.AntiDiagonal[antiDiagonalIndex]) - (GetBitAt(index) << 1)) & Masks.AntiDiagonal[antiDiagonalIndex];
        Bitboard bottomRight = (((occupied & Masks.AntiDiagonal[antiDiagonalIndex]).Reverse() - (GetBitAt(index).Reverse() << 1)).Reverse()) & Masks.AntiDiagonal[antiDiagonalIndex];
    ...
    public static Bitboard GetUnsafeSquares(Board board, bool color) {
        ...
        // Enemy pawns
        Bitboard pawns = board.Type[PieceType.Pawn] & board.Color[!color];
        if (color) {
        ...
        // Enemy knights
        Bitboard knights = board.Type[PieceType.Knight] & board.Color[!color];
        ...
        Bitboard straight = (board.Type[PieceType.Rook] | board.Type[PieceType.Queen]) & board.Color[!color];
        ...
        Bitboard diagonal = (board.Type[PieceType.Bishop] | board.Type[PieceType.Queen]) & board.Color[!color];
        ...
        // Enemy king
        Bitboard kings = board.Type[PieceType.King] & board.Color[!color];
    ...
    public static bool IsInCheck(Board board, bool color) {
        ...
        return !(unsafeSquares & board.Type[PieceType.King] & board.Color[color]).IsEmpty;
    }
}